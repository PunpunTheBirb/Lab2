...
public class PathManagerEditor : Editor
{
    ...
    public bool DrawPoint(Waypoint p)
    {
        ...
        {
            Vector3 currPoss = p.GetPos();
            float handleSize = HandleUtility.GetHandleSize(currPoss);
            if (Handles.Button(currPoss, Quaternion.identity, 0.25f * handleSize, 0.25f * handleSize, Handles.SphereHandleCap))
            {
                isChanged = true;
                selectedPoint = p;
            }
